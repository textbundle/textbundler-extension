...
| DD-16 | Markdown conversion in content script, not background service worker | Background-owned conversion per TASK-018b original design | Chrome MV3 service workers lack `DOMParser`, causing Turndown to crash with `document is not defined`. Content script has full DOM access. `ExtractionResult` carries pre-converted `markdown` + `imageMap` to background. Background still owns image download, patching, frontmatter, packaging, and download trigger. | FR-002, NFR-003 |
...
| 1.4 | 2026-02-15 | Agent | Add DD-16: content-script markdown conversion (Chrome MV3 DOMParser limitation) |
